...
public class FindJobsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isJson || request.getParameter("skills") != null || request.getParameter("role") != null) {
    ...
    private void handleSearch(HttpServletRequest request, HttpServletResponse response, boolean isJson)
        ...
        // Fetch prioritized matching jobs via JobMatchingService
        List<JobMatchingService.JobCard> matchingJobs = JobMatchingService.findMatchingJobs(skillTokens, role, experience, salary);
        ...
        out.println("                <a href='job-search.html' class='btn btn-secondary btn-sm'>← Back to Search</a>");
        out.println("                <a href='login.html' class='btn btn-outline btn-sm'>Logout</a>");
        out.println("            </div>");
        out.println("        </div>");
        out.println("    </header>");
        ...
        out.println("                <h2>Your Search Preferences</h2>");
        out.println("            </div>");
        ...
        out.println("        <div class='results-section-header' style='display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;'>");
        out.println("            <div>");
        out.println("                <h2>Recommended Opportunities (" + matchingJobs.size() + " Found)</h2>");
        out.println("                <p style='font-size:13px; color:var(--text-muted); margin-top:4px;'>Filtered & prioritized by skill overlap (highest matching skills first).</p>");
        out.println("            </div>");
        out.println("            <a href='job-search.html' class='btn btn-outline btn-sm'>🔍 Adjust Filters</a>");
        out.println("        </div>");
        ...
        for (JobMatchingService.JobCard job : matchingJobs) {
            ...
            out.println("                                <span class='ai-badge-label'>" + (job.matchCount > 0 ? job.matchCount + " Skills Matched" : job.matchTierLabel) + "</span>");
            out.println("                            </div>");
            out.println("                        </div>");
            ...
            out.println("                    <span class='badge badge-verified'>✓ Verified Opening</span>");
            out.println("                </div>");
            ...
            // Card Action Buttons: AI Skill Analysis & Apply
            ...
            out.println("                            data-target-role='" + escapeHtml(role) + "'");
            out.println("                            data-user-exp='" + escapeHtml(experience) + "'");
            out.println("                            onclick='openAiSkillModal(this)'>");
            out.println("                        ✨ AI Skill Analysis");
            out.println("                    </button>");
            out.println("                    <a href='" + job.applyUrl + "' target='_blank' rel='noopener noreferrer' class='btn btn-apply'>");
            out.println("                        Apply on " + escapeHtml(job.companyName) + " ↗");
            out.println("                    </a>");
            out.println("                </div>");
            out.println("            </div>");
        }
        ...
        if (matchingJobs.isEmpty()) {
            ...
            out.println("            <p>We couldn't find openings matching your selected skills: <strong>" + escapeHtml(skills) + "</strong>. Try removing some skills or adding complementary tools.</p>");
        ...
        // AI Career Intelligence Modal Markup
